...
oTitle <- fluidRow(
  column(width = 12,
         tags$div(
           HTML(paste(tags$span(style="font-weight:bold; color:#12pleas454f", "Diabetes Predictor"))), style = "font-size:1.8em",align = "center"),
  ))
oSource <- fluidRow(
  column(width = 12,
         tags$div(
...
oInfo <- fluidRow(
  column(width =12,
         ...
         tags$div(
           HTML(paste(tags$span(style="font-weight:bold; color:#337a87", "Diabetes Predictor, "),
         ...
         tags$div(
           HTML(paste("User only needs to provide information such as ",
                      tags$span(style="font-weight:bold; color:#337a87", "age, number of pregnancy, blood pressure, glucose level, skin thickness, Insulin, BMI, Diabetes Pedigree Function. "),
                      sep = "")), style = "font-size:1.2em"
         ),hr(style = "border-color:white;margin-bottom:0.25em"),
...
ui <- dashboardPage(
  dashboardHeader(title = 'Diabetes Predictor'),
...
server <- function(input, output, session) {
  ...
  training_set<-training_set %>%
    filter(Glucose >0 , BloodPressure >25 , SkinThickness >0 , BMI >0 )
 ...
 # library(class)
  ...
  output$ibox = renderValueBox({
    valueBox(
      "Prediction:", ifelse( final_prediction() >= 0.5 , yes = 'YES', no = 'NO'), color = ifelse(final_prediction() >= 0.5, 'red','green')
    )
  })
  ...
  output$cplot<- renderPlot({
    ...
    ggplot(new_df, aes(x = x, y = y))+
      theme_bw()+
      geom_violin(aes(fill=x), draw_quantiles = c(0.25, 0.5, 0.75))+
